...
void HexView::row_matches(const Matches &matches, size_t offset, bool *hit, bool *current) const {
    std::fill(hit, hit + columns, false);
    std::fill(current, current + columns, false);
    ...
    for (; it != matches.offsets.end() && *it < offset + columns; ++it) {
        const bool is_current = static_cast<size_t>(it - matches.offsets.begin()) == matches.current;

        for (size_t col = 0; col < columns; col++) {
            if (!within(offset + col, *it, matches.len))
                continue;

            hit[col] = true;
            current[col] = current[col] || is_current;
        }
    }
}
...
void HexView::draw_row(ImDrawList *draw, const ImVec2 &pos, float glyph, float line, const struct Stream &stream, size_t offset, const Matches &matches) {
    ...
    this->row_matches(matches, offset, hit, current);
